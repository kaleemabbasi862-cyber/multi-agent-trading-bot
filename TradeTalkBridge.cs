...
namespace cAlgo.Robots
{
    ...
    public class TradeTalkBridge : Robot
    {
        ...
        private const double HARD_DRAWDOWN_LIMIT_USD = -5.00;     // Halt all new trades if open loss reaches -.00
        private const double MIN_BALANCE_FOR_METALS_USD = 200.00; // Hard ban on XAU/XAG/BTC on balances < 
        private const int MAX_ACTIVE_OPEN_POSITIONS = 1;          // Strict hard cap: Maximum 1 open trade at all times
        ...
        protected override void OnStart()
        {
            ...
            Print(=================================================);
            Print(TradeTalk.AI Autonomous Bridge - HARD RISK ENFORCEMENT ACTIVE);
            Print(Account Number:  + Account.Number);
            Print(string.Format(CultureInfo.InvariantCulture, Live Balance: {1} | Equity: , Account.Balance, assetName, Account.Equity));
            Print(string.Format(CultureInfo.InvariantCulture, Hard Drawdown Cutoff: , HARD_DRAWDOWN_LIMIT_USD));
            Print(string.Format(CultureInfo.InvariantCulture, Metals / Crypto Minimum Balance: , MIN_BALANCE_FOR_METALS_USD));
            Print(Max Open Positions:  + MAX_ACTIVE_OPEN_POSITIONS);
            Print(Target Server:  + ServerUrl);
            Print(=================================================);

            // Immediate verification of all positions upon startup
        ...
        protected override void OnTimer()
        {
            try
            {
                // 1. Post-Execution & Open Trade Guard: Ensure every open position has verified SL or immediately close
                EnsureAllPositionsProtected();
                ...
                // 4. Check Hard Drawdown cutoff before polling/executing orders
                double totalUnrealizedPnL = GetTotalUnrealizedPnL();
                bool isDrawdownHalted = totalUnrealizedPnL <= HARD_DRAWDOWN_LIMIT_USD || (Account.Equity - Account.Balance) <= HARD_DRAWDOWN_LIMIT_USD;

                if (isDrawdownHalted)
                {
                    Print(string.Format(CultureInfo.InvariantCulture, 
                        🚨 [HARD DRAWDOWN CUTOFF ACTIVE] Unrealized PnL: <= . New trade executions halted., 
                        totalUnrealizedPnL, HARD_DRAWDOWN_LIMIT_USD));
                    return;
                }

                // 5. Poll & Execute pending approved orders only if safe
        ...
        /// If SL attachment is rejected by broker, IMMEDIATELY CLOSES THE POSITION to protect account equity.
        /// </summary>
        private void EnsureAllPositionsProtected()
        {
            try
            {
                // Copy positions to avoid collection modified exception during close
                ...
                foreach (var pos in openPositions)
                {
                    if (pos.StopLoss == null || pos.StopLoss <= 0)
                    {
                        ...
                        // Dynamic Stop Loss Distance: at least (Spread * 3.0) and at least 30 pips
                        ...
                        {
                            targetSl = Math.Round(sym.Bid - Math.Max(minRequiredDistance, pipSize * 50.0), digits);
                            targetTp = Math.Round(sym.Ask + Math.Max(minRequiredDistance * 2.0, pipSize * 100.0), digits);
                        }
                        else
                        {
                            targetSl = Math.Round(sym.Ask + Math.Max(minRequiredDistance, pipSize * 50.0), digits);
                            targetTp = Math.Round(sym.Bid - Math.Max(minRequiredDistance * 2.0, pipSize * 100.0), digits);
                        }

                        Print(string.Format(🛡️ [Emergency SL Attachment] Attempting SL for #{0} ({1} {2}) @ SL: {3}, TP: {4}, 
                            pos.Id, pos.SymbolName, pos.TradeType, targetSl, targetTp));

                        TradeResult modResult = ModifyPosition(pos, targetSl, targetTp);

                        // If modify failed or position remains without verified SL, FAIL-SAFE EMERGENCY AUTO-CLOSE
                        if (modResult == null || !modResult.IsSuccessful || pos.StopLoss == null || pos.StopLoss <= 0)
                        {
                            Print(string.Format(🚨 [CRITICAL FAIL-SAFE AUTO-CLOSE] Position #{0} ({1}) is unprotected and SL attachment failed ({2}). Closing position immediately to protect account equity!, 
                                pos.Id, pos.SymbolName, modResult != null ? modResult.Error.ToString() : Rejected));
                            
                            ClosePosition(pos);
                        }
                        else
                        {
                            Print(string.Format(✅ [Protection Verified] Position #{0} secured with SL: {1}, TP: {2}, pos.Id, pos.StopLoss, pos.TakeProfit));
 ...
 // --- CONSTRAINT 1: HARD DRAWDOWN CUTOFF CHECK ---
 double unrealizedLoss = GetTotalUnrealizedPnL();
 if (unrealizedLoss <= HARD_DRAWDOWN_LIMIT_USD || (Account.Equity - Account.Balance) <= HARD_DRAWDOWN_LIMIT_USD)
 {
 Print(string.Format(CultureInfo.InvariantCulture, 
 🚫 [HARD RISK REJECT] Drawdown cutoff exceeded (PnL:  <= ). Order {2} {3} blocked., 
 unrealizedLoss, HARD_DRAWDOWN_LIMIT_USD, action, symbolStr));
 return;
 }
 ...
 {
 Print(string.Format(🚫 [HARD RISK REJECT] Max Open Positions ({0}) reached. Currently open: {1}. Order {2} {3} blocked., 
 MAX_ACTIVE_OPEN_POSITIONS, Positions.Count, action, symbolStr));
 return;
 }
 ...
 {
 if (openPos.SymbolName.Equals(targetSymbol.Name, StringComparison.OrdinalIgnoreCase) ||
 openPos.SymbolName.StartsWith(symbolStr.Replace(USD, ), StringComparison.OrdinalIgnoreCase))
 {
 Print(🚫 [HARD RISK REJECT] An active position already exists on  + targetSymbol.Name + . Opposing / Hedging orders blocked.);
 return;
 }
 }

 // --- CONSTRAINT 2: BAN METALS / COMMODITIES / CRYPTO ON MICRO BALANCES (< ) ---
 string upperSym = targetSymbol.Name.ToUpperInvariant();
 bool isMetalOrCommodity = upperSym.Contains(XAU) || upperSym.Contains(GOLD) ||
 upperSym.Contains(XAG) || upperSym.Contains(SILVER) ||
 upperSym.Contains(BTC) || upperSym.Contains(OIL) ||
 upperSym.Contains(US30) || upperSym.Contains(NAS100) || upperSym.Contains(ETH);

 if (isMetalOrCommodity && Account.Balance < MIN_BALANCE_FOR_METALS_USD)
 {
 Print(string.Format(CultureInfo.InvariantCulture, 
 🚫 [HARD RISK REJECT] Commodity/Metal/Crypto trading is BANNED on accounts under . Current Balance: . Only 0.01 Forex allowed., 
 MIN_BALANCE_FOR_METALS_USD, Account.Balance));
 return;
 }
 ...
 // Micro Account strict lot enforcement: never exceed 0.01 on accounts < 
 if (Account.Balance < MIN_BALANCE_FOR_METALS_USD && lotSize > 0.01)
 {
 lotSize = 0.01;
 }
 ...
 // Dynamic Stop-Level Enforcement (Strictly at least Spread * 3.0 and at least 30 pips)
 ...
 {
 validSl = targetSymbol.Bid - Math.Max(minDistance, pipSize * 50.0);
 }
 if (validTp <= 0 || validTp <= (targetSymbol.Ask + minDistance))
 {
 validTp = targetSymbol.Ask + Math.Max(minDistance * 2.0, pipSize * 100.0);
 }
 }
 else
 {
 ...
 {
 validSl = targetSymbol.Ask + Math.Max(minDistance, pipSize * 50.0);
 }
 if (validTp <= 0 || validTp >= (targetSymbol.Bid - minDistance))
 {
 validTp = targetSymbol.Bid - Math.Max(minDistance * 2.0, pipSize * 100.0);
 }
 }
 ...
 // 1. Attempt market execution with embedded SL/TP pips
 TradeResult result = ExecuteMarketOrder(tradeType, targetSymbol.Name, volumeInUnits, TradeTalk.AI, slPips, tpPips);
 
 // 2. Auto-Recovery Fallback: If broker rejects embedded SL/TP, execute clean market order and modify immediately
 ...
 Print(string.Format(🟢 cTrader Order FILLED! Position ID: #{0} | Entry: {1} | SL: {2} | TP: {3}, 
 pos.Id, pos.EntryPrice, pos.StopLoss, pos.TakeProfit));

 // --- CONSTRAINT 4: NO TRADE WITHOUT VERIFIED STOP LOSS ---
 if (pos.StopLoss == null || pos.StopLoss <= 0)
 {
 Print(string.Format(🛡️ Attaching immediate required Stop Loss to #{0}: SL={1}, TP={2}, pos.Id, validSl, validTp));
 TradeResult modRes = ModifyPosition(pos, validSl, validTp); 
 
 // Fail-Safe: If SL attachment failed and position remains unprotected, CLOSE IMMEDIATELY within 2s
 if (modRes == null || !modRes.IsSuccessful || pos.StopLoss == null || pos.StopLoss <= 0)
 {
 Print(string.Format(🚨 [CRITICAL FAIL-SAFE AUTO-CLOSE] Position #{0} opened WITHOUT verified Stop Loss! Broker rejected SL attachment ({1}). Closing immediately!, 
 pos.Id, modRes != null ? modRes.Error.ToString() : Error));
 ...
 // 4. Notify TradeTalk server with authentic cTrader Position ID
