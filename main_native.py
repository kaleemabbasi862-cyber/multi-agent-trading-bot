...
from app.services.logger import setup_structured_logging
...
async def autonomous_market_scanner_loop():
    ...
    last_scan_ts = 0.0
    last_emitted_signal_ts = 0.0
    last_emitted_action = ""
    logger.info("[Autonomous Engine] 🚀 24/7 Multi-Timeframe (15m/1H) Protected Market Scanner & Execution Engine Initialized")
    while True:
        try:
            now = time.time()
            # Active scan cadence: check every 10 seconds
            if now - last_scan_ts >= 10.0:
                ...
                if cur_settings.get("auto_trade_enabled", False):
                    ...
                    cooldown_req = getattr(settings, "EXECUTION_COOLDOWN_SECONDS", 30)
                    if last_exec > 0 and time_since_exec < cooldown_req:
                        rem = int(cooldown_req - time_since_exec)
                        logger.debug(f"[Autonomous Scanner] ⏳ Execution Cooldown Active ({rem}s / {cooldown_req}s remaining before next scan).")
                        await asyncio.sleep(5)
                        continue
                    ...
                    # -------------------------------------------------------------
                    # A. ACTIVE POSITION MANAGEMENT (Autonomous Position Sentinel)
                    # -------------------------------------------------------------
                    if len(open_positions) >= ctrader_cloud_gateway.MAX_ACTIVE_OPEN_POSITIONS:
                        try:
                            ...
                            sentinel_actions = position_sentinel.evaluate_open_positions()
                            if sentinel_actions:
                                for act_item in sentinel_actions:
                                    logger.info(f"[Autonomous Position Sentinel] 🛡️ Action Executed: {act_item.get('action')} on #{act_item.get('position_id')}")
                        except Exception as se:
                            logger.debug(f"[Position Sentinel Error]: {se}")
                        await asyncio.sleep(5)
                        continue
                    ...
                    if market_data and market_data.get("price"):
                        ...
                        tf_candles = get_multi_timeframe_candles(cur_sym)

                        # Run Complete Pre-Trade Intelligence Scan
                        pretrade_scan = pretrade_intelligence_engine.scan_market(
                            symbol=cur_sym,
                            live_tick={"bid": bid, "ask": ask, "spread": spread_pips},
                        ...
                        quality_score = pretrade_scan.get("quality_score", {}).get("score", 0.0)
                        ...
                        # 4. Anti-Churn Signal Pacing Guard (prevent generating duplicate signals within 30s)
                        time_since_signal = now - last_emitted_signal_ts
                        if act == last_emitted_action and time_since_signal < 30.0:
                            await asyncio.sleep(5)
                            continue

                        # Dynamic SMC-calibrated SL/TP ($6.00 SL / $12.00 TP on Gold)
                        ...
                        # 6. Live Safety Gate Check
                        is_safe, safety_msg, _ = live_safety_gate.evaluate_order_safety(
                            symbol=cur_sym,
                            action=act,
                            volume=cur_lot,
                            entry_price=p,
                        ...
                        if not is_safe:
                            logger.debug(f"[Autonomous Scanner] 🛑 Safety Gate Block: {safety_msg}")
                            await asyncio.sleep(5)
                            continue
                        ...
                        sig = SignalPayload(
                            ...
                            volume=cur_lot,
                            timeframe="15m & 1H",
                            strategy_name=f"{cur_sym}_{setup_type}",
...
async def receive_tradingview_webhook(
    ...
    live_p = float(market_data.get("price") or market_data.get("bid") or 0.0)
    ...
    vol_state = volatility_engine.get_symbol_volatility(symbol)
    def_sl_dist = vol_state.get("recommended_sl_distance", 6.0)
    def_tp_dist = round(def_sl_dist * 2.0, 5)

    sl = float(payload_json.get("stop_loss") or payload_json.get("sl") or (entry - def_sl_dist if action == "BUY" else entry + def_sl_dist))
    tp = float(payload_json.get("take_profit") or payload_json.get("tp") or (entry + def_tp_dist if action == "BUY" else entry - def_tp_dist))
...
async def chat_copilot(req: CopilotChatRequest):
    ...
    return copilot_agent.execute_copilot_intent(user_msg, sys_state)
...
@app.get("/api/pretrade/status")
@app.get("/api/pretrade/intelligence")
async def get_pretrade_status(symbol: Optional[str] = None):
    """
    Returns real-time Pre-Trade Intelligence & Chart Scanner telemetry.
    Includes MTF confluence, SMC structure, session data, setup model, and 0-100 quality score.
    Automatically re-evaluates if cache is older than 10 seconds.
    """
    cur_sym = symbol or settings_manager.get_active_symbol() or "XAUUSD"
    cached = PRETRADE_LATEST_STATE.get(cur_sym)
    now_ts = time.time()
    
    if cached and isinstance(cached, dict):
        scan_ts = cached.get("timestamp_epoch", 0.0)
        if (now_ts - scan_ts) < 10.0:
            return cached
    ...
    scan["timestamp_epoch"] = now_ts
    PRETRADE_LATEST_STATE[cur_sym] = scan
    return scan

@app.post("/api/pretrade/scan")
async def trigger_pretrade_scan(symbol: Optional[str] = None):
